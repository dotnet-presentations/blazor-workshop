...
Open `Address.cs` from the `BlazingPizza.Server` project, and put a `[Required]` attribute onto each of the properties except for `Id` (which is implictly required as it's the primary key) and `Line2`, since not all addresses need a second line. You can also place some `[MaxLength]` attributes if you wish, or any other `DataAnnotations` rules:
